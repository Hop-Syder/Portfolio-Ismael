...
function PageLoad() {
  ...
  // Animation d'incrémentation du pourcentage
...
$(document).ready(function () {
  ...
  // Popup Newsletter Automatique (Affiche après 2 secondes)
  setTimeout(function () {
    var popupChecked = localStorage.getItem('newsletter-popup-shown-v1');
    $('#newsletter-popup').removeClass('mfp-hide').addClass('active');
  }, 2000);
  ...
  $('.newsletter-popup__close').on('click', function () {
    ...
    setTimeout(function () {
      $('#newsletter-popup').addClass('mfp-hide');
    }, 300);
  });

});

// Google Maps (chargement dynamique pour éviter de committer la clé)
window.marker = null;

function loadGoogleMaps(apiKey) {
  return new Promise(function (resolve, reject) {
    if (!apiKey) {
      return reject(new Error('Aucune clé Google Maps fournie.'));
    }
    ...
    script.onerror = function () { return reject(new Error('Échec du chargement de Google Maps.')); };
...
function bootGoogleMaps() {
  var apiKey = window.GMAPS_API_KEY || '';

  if (!apiKey) {
    console.warn('Google Maps API key manquante. Ajoutez window.GMAPS_API_KEY dans js/config.js.');
    return;
  }
...
window.addEventListener('load', bootGoogleMaps);
